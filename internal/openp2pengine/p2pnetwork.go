...
func (pn *P2PNetwork) reconnectDelay() time.Duration {
	if pn.loginMaxDelaySeconds <= 0 {
		return ClientAPITimeout
	}
	return ClientAPITimeout + time.Duration(rand.Intn(pn.loginMaxDelaySeconds))*time.Second
}
...
func (pn *P2PNetwork) handleMessage(msg []byte) {
	...
	case MsgSDWAN:
		// OPL embeds only OpenP2P port tunnels; SD-WAN is provided by its WireGuard layer.
	default:
...
func (pn *P2PNetwork) shutdown() {
	pn.shutdownOnce.Do(func() {
		...
		pn.allTunnels.Range(func(id, value interface{}) bool {
			tunnel := value.(*P2PTunnel)
			tunnel.setRun(false)
			if tunnel.conn != nil {
				tunnel.conn.Close()
			}
			tunnel.wg.Wait()
			pn.allTunnels.Delete(id)
			return true
		})
		pn.apps.Range(func(id, value interface{}) bool {
			value.(*p2pApp).Close()
			pn.apps.Delete(id)
			return true
		})
