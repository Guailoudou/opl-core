...
func (app *p2pApp) Start(isClient bool) {
	...
	app.PreCalcKeyBytes()
	if isClient {
		app.goWorker(func() { app.daemonP2PTunnel() })
	}

}

func (app *p2pApp) goWorker(worker func()) {
	app.workerMtx.Lock()
	defer app.workerMtx.Unlock()
	if !app.running.Load() {
		return
	}
	app.wg.Add(1)
...
func (app *p2pApp) wait(delay time.Duration) bool {
	select {
	case <-app.done:
		return false
...
func (app *p2pApp) daemonDirectTunnel(idx int) error {
	...
	if err != nil {
		...
		app.statusError.Store(err.Error())
	...
	if app.Tunnel(idx) != nil {
		...
		app.once.Do(func() {
			app.goWorker(func() { app.listen() })
			// memapp also need
			for i := app.relayIdxStart; i < app.tunnelNum; i++ {
				idx := i
				app.goWorker(func() { app.relayHeartbeatLoop(idx) })
			}
...
func (app *p2pApp) buildDirectTunnel(idx int) error {
	...
	gLog.d("appid:%d buildDirectTunnel push %s ServerSideSaveMemApp: %s", app.id, app.config.LogPeerNode(), req2.logSummary())
...
func (app *p2pApp) buildRelayTunnel(idx int) error {
	...
	gLog.d("appid:%d buildRelayTunnel push %s relay ServerSideSaveMemApp: %s", app.id, config.LogPeerNode(), req2.logSummary())
	// }
...
func (app *p2pApp) relayHeartbeatLoop(idx int) {
	...
	for app.running.Load() {
		if app.Tunnel(idx) == nil || !app.Tunnel(idx).isRuning() {
			if !app.wait(TunnelHeartbeatTime) {
				return
			}
			continue
		}
