...
func (s ServerSideSaveMemApp) logSummary() string {
	return fmt.Sprintf("from=%s node=%s tunnelID=%d relayTunnelID=%d appID=%d relayIndex=%d tunnelNum=%d srcPort=%d",
		s.From, s.Node, s.TunnelID, s.RelayTunnelID, s.AppID, s.RelayIndex, s.TunnelNum, s.SrcPort)
}
