...
const relayTOTPStep int64 = 30

func relayTokenValid(code, token uint64, timestamp int64) bool {
	if code == 0 {
		return false
	}
	if code == token {
	...
	return code == relayTOTP(token, timestamp) ||
		code == relayTOTP(token, timestamp-relayTOTPStep) ||
		code == relayTOTP(token, timestamp+relayTOTPStep)
}

func relayTOTP(token uint64, timestamp int64) uint64 {
	var key, message [8]byte
	binary.LittleEndian.PutUint64(key[:], token)
	binary.LittleEndian.PutUint64(message[:], uint64(timestamp/relayTOTPStep))
...
func handlePush(subType uint16, msg []byte) error {
	...
	case MsgPushServerSideSaveMemApp:
		...
		gLog.d("handle MsgPushServerSideSaveMemApp:%s", req.logSummary())
		...
		return nil
	case MsgPushUpdate:
		gLog.i("ignore remote update in embedded mode")
		return nil
	case MsgPushRestart:
		gLog.i("MsgPushRestart")
		GNetwork.close(false)
		return nil
	...
	case MsgPushSwitchApp:
		...
		if err = json.Unmarshal(msg[openP2PHeaderSize:], &app); err != nil {
			gLog.e("Unmarshal %v:%s", reflect.TypeOf(app), err)
			return err
		}
	...
	case MsgPushSDWanRefresh:
		gLog.i("ignore SD-WAN refresh in embedded mode")
...
func handleNat4Detect(msg []byte) (err error) {
	...
	if err = json.Unmarshal(msg[openP2PHeaderSize:], &nd); err != nil {
		gLog.e("Unmarshal %v:%s", reflect.TypeOf(nd), err)
		return err
	}
...
func handleEditApp(msg []byte) (err error) {
	...
	if err = json.Unmarshal(msg[openP2PHeaderSize:], &newApp); err != nil {
		gLog.e("Unmarshal %v:%s", reflect.TypeOf(newApp), err)
		return err
	}
...
func handleConnectReq(msg []byte) (err error) {
	...
	if relayTokenValid(req.Token, gConf.Network.Token, time.Now().Unix()-GNetwork.dt/int64(time.Second)) { // localTs may behind, auto adjust ts
